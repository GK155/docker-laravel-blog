<div class="w-full">
    <form action="/categories/update" method="post" class="flex-col justify-center items-center">

        <input type="hidden" name="_token" value="{{ csrf_token() }}">
        <input type="hidden" name="slug" value="{{ $category->slug }}"/>
        <div class="block text-sm font-medium text-gray-700 mt-5">
            <input required="required" value="{{ $category->title }}" placeholder="Enter title here" type="text"
                   name="title"
